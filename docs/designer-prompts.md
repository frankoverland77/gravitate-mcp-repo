# Example Designer Prompts

Example prompts for working with Claude Code to create demos using the Excalibrr component library.

## 🚀 Demo Creation

### Creating Form Demos

- **"Create a product form with name, price, category, and description fields"**
- **"Make a customer registration form with validation"**
- **"Build an inventory management form with SKU, quantity, and location"**
- **"Create a user profile form with email, phone, and preferences"**

### Creating Grid Demos

- **"Create a product inventory grid with editable quantity columns"**
- **"Make a customer management grid showing name, email, status, and signup date"**
- **"Build a simple data grid for displaying order information"**

### Working with Themes

- **"Change the ProductGrid theme to PE"**
- **"Apply BP theme to my form demo"**
- **"Switch the current demo to OSP theme"**
- **"Show me all available themes"**

---

## 📚 Component Registry

### Browsing Components

- **"List all available components"**
- **"Show me all form components"**
- **"What layout components are available?"**
- **"List simple components"**
- **"Show me complex components"**

### Searching Components

- **"Search for grid components"**
- **"Find button components"**
- **"Search for editable components"**
- **"Find components with modal"**

### Getting Component Details

- **"Show me GraviGrid documentation"**
- **"How do I use the Modal component?"**
- **"What props does GraviButton accept?"**
- **"Get details for Horizontal component"**
- **"Show me Texto examples"**

### Installing/Using Components

- **"How do I add GraviGrid to my project?"**
- **"Install the Modal component"**
- **"Show me how to use Select"**
- **"What dependencies does GraviGrid need?"**

---

## 📖 Component Information

### Learning About Components

- **"Show me how to use the GraviGrid component"**
- **"What are the main props for Horizontal and Vertical layout components?"**
- **"How do I use GraviButton with different themes?"**
- **"Show me examples of Texto component usage"**

### Form Components

- **"How do I create form fields using Excalibrr components?"**
- **"What validation options work with form demos?"**
- **"Show me proper form layout patterns"**
- **"How do I handle form submissions in demos?"**

---

## 🎨 Design-Focused Questions

### Layout & Structure

- **"How do I create a two-column layout?"**
- **"What's the best way to center content horizontally and vertically?"**
- **"Show me how to build a grid with controls"**
- **"How do I create a responsive layout?"**

### Data Display

- **"How do I display tabular data with filtering?"**
- **"What components should I use for a dashboard?"**
- **"Show me how to create a data table with actions"**
- **"How do I display tags or badges?"**

### Forms & Interactions

- **"What form components are available?"**
- **"How do I create a date picker?"**
- **"Show me button variations"**
- **"How do I handle form validation?"**

---

## 🛠️ Demo Management

### Managing Existing Demos

- **"Clean up the test demo I created"**
- **"Remove the ProductForm demo"**
- **"List all available demos"**
- **"Show me what demos are currently in the project"**

### Modifying Demos

- **"Add an editable quantity column to the ProductGrid"**
- **"Make the price column editable with number validation"**
- **"Add a new field to the inventory form"**
- **"Change the form layout to horizontal"**

### Troubleshooting

- **"Fix the syntax error in my ProductGrid column definitions"**
- **"Why isn't my form demo showing up in the navigation?"**
- **"The theme isn't applying correctly to my demo"**
- **"Help me debug the column renderer issue"**

---

## 🚀 Advanced Usage

### Component Relationships

- **"What components work well together?"**
- **"Show me how GraviGrid is typically used with other components"**
- **"What's a complete example of a data management interface?"**

### Customization

- **"How do I customize the styling of components?"**
- **"What CSS properties can I override?"**
- **"Show me advanced configuration options"**

---

## 💡 Creative Combinations

### Ask for Design Patterns

- **"Create a master-detail view pattern"**
- **"Show me a typical admin dashboard layout"**
- **"Design a data entry form with validation"**
- **"Build a user management interface"**

### Specific Use Cases

- **"Create a file upload interface with progress indicators"**
- **"Design a settings panel with multiple sections"**

---

## 🎯 Pro Tips

1. **Be Specific**: Instead of "create a form", ask "create a product form with name, price, and category fields"

2. **Include Field Details**: "Create an inventory form with SKU (text), quantity (number), and active status (switch)"

3. **Specify Themes**: "Apply OSP theme to my demo" rather than just "change the theme"

4. **Use Clear Names**: "ProductForm" and "CustomerGrid" are clearer than "MyForm" or "TestGrid"

5. **Test Incrementally**: Start with simple demos, then add complexity with modify tools

6. **Use Component Registry**: Ask "show me GraviGrid documentation" before building to understand available props

---

## Available Claude Code Skills

- `/excalibrr` - Main development assistant for Excalibrr demos
- `/start-feature` - Scaffold a new feature with project context
- `/excalibrr-create` - Create a new component demo
- `/excalibrr-patterns` - Reference Excalibrr design patterns
- `/bulk-change` - GraviGrid bulk change reference
